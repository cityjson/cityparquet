    ...
    pub fn add_cjfeature(&mut self, cjf: &mut CityJSONFeature) {
        ...
        if let Some(cjf_app) = &cjf.appearance {
            ...
            if let Some(cjf_v_tex) = &cjf_app.vertices_texture {
                // Accumulated `vertices_texture` pool length BEFORE this
                // feature's own UV-vertices are appended below — the
                // texture analogue of `g_offset` above, which does the same
                // for 3D vertices. See `vendor/cjseq/PATCHES.md`: upstream
                // used `cjf_v_tex.len()` here (this feature's OWN local
                // UV-vertex count), not the pool's accumulated length.
                t_offset = self
                    .appearance
                    .as_ref()
...
impl Geometry {
    ...
    fn update_texture(
        ...
        offset: usize,
        // `true` (the `get_cjfeature`/`get_metadata` callers, slicing a doc
        // DOWN into a compact per-feature `vertices_texture` array): a
        // fresh, gapless, first-encounter-order index is assigned per
        // distinct UV vertex, because the caller then gathers only the
        // REFERENCED entries into a freshly `resize`d array by that new
        // compacted position (see e.g. `t_new_vertices.resize(t_v_oldnew.len(), ...)`
        // just after the calls at lines ~101/167/184).
        // `false` (the `add_cjfeature` caller, merging a feature's whole
        // local `vertices_texture` array UP into the accumulated global
        // pool via `add_vertices_texture`, which appends it unchanged, at
        // its original local positions): the rebased index must be
        // `thevalue + offset` directly. See `vendor/cjseq/PATCHES.md`.
        compact: bool,
    ) {
