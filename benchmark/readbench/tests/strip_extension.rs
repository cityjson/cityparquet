...
fn mono_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("..")
        .canonicalize()
        .expect("the monorepo root is two levels above benchmark/readbench")
}

fn read(relative: &str) -> String {
    let path = mono_root().join(relative);
...
/// The single line of `benchmark/scripts/readbench_prepare.sh` that owns its copy of
...
// ---------------------------------------------------------------------------
// 2. benchmark/scripts/readbench_prepare.sh
// ---------------------------------------------------------------------------

#[test]
fn the_prepare_script_strips_identically() {
    let script = read("benchmark/scripts/readbench_prepare.sh");
    ...
    assert_matches_the_table(
        "benchmark/scripts/readbench_prepare.sh",
...
fn the_prepare_script_carries_the_same_extension_list() {
    ...
    assert_eq!(
        ...
        "benchmark/scripts/readbench_prepare.sh's KNOWN_INPUT_EXTENSIONS has drifted from the Rust one"
    );
}
