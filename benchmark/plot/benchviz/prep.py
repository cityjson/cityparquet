...
def load_ordering(inputs: Inputs) -> list[dict]:
    ...
    for path in _dataset_csvs(inputs.ordering_dir):
        ...
        groups: dict[str, dict[str, dict[str, str]]] = {}
        for row in rows:
            if COLD_RE.search(row["notes"]) or row["scenario"] in RETIRED_SCENARIOS:
                continue
...
def load_scaling(inputs: Inputs) -> dict:
    ...
    for name, rows in slices.items():
        ...
        for row in rows:
            if (
                COLD_RE.search(row["notes"])
                or row["format"] not in KNOWN_FORMATS
                or row["scenario"] in RETIRED_SCENARIOS
            ):
                continue
    ...
    for path in _dataset_csvs(inputs.scaling_ordering_dir):
        ...
        for row in _read_rows(path, READ_COLUMNS):
            if (
                row["format"] not in (ORDERING_BASELINE, ORDERING_VARIANT)
                or row["scenario"] in RETIRED_SCENARIOS
            ):
                continue
            groups.setdefault(_scenario_key(row), {})[row["format"]] = row
        ...
        records.append(
            {
                ...
                "time_mad_s": safe_float(row.get("time_mad_s")),
