...
package of the `benchmark/plot` uv project, and takes its input and output locations
...
- **The format axis is the harness's, not this package's.** The views plot
  ...
  own run (`Format::ORDERING_SET`, `benchmark/formats/ordering_results/`), its own baseline
...
`benchmark/formats/READ_BENCHMARK.md` (lines under that heading until the
next `##`) splitting on the numbered items, and the "## Baseline geometry
coverage" section from `benchmark/formats/README.md`. Extraction is by
...
```
benchmark/plot/                 # uv project, shared with readbench_plot
