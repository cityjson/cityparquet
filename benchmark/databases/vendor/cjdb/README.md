...
contrived about it. On the Delft fixture published at
`https://storage.googleapis.com/cityjson/delft.city.jsonl` (2231 CityObjects;
not committed here), **9 of the 1116 BuildingParts** that reach this code
path have two or more tied faces, one losing up to 8 of them
(`UPSTREAM-BUG-REPORT.md`). The practical
...
`UPSTREAM-BUG-REPORT.md` is a draft upstream bug report with a minimal
reproduction.
...
- `<dataset>.manifest.json`'s `patches.cjdb` (built by
  `citybench.systems.cjdb.patch_disclosure()`) and `versions.cjdb`, written
  beside every results CSV (by default in `benchmark/runs/databases/results/`).
- `benchmark/databases/README.md`.
- This file.

## The patch

`ground-surfaces-tie.patch` — a unified diff applying with `patch -p1` from
the extracted sdist root. Its `cjdb/modules/geometric.py` hunk changes exactly
one thing: `ground_surfaces` becomes a
...
count-weighted mean would be a better split rule is a separate question the
patch does not address.

## Large-import batching

The patch's `cjdb/modules/importer.py` hunk changes only the importer
transport for large CityJSONSeq files. Stock cjdb builds one PostgreSQL `INSERT` containing every CityObject
...
change at a *fixed* path (even
`uv cache clean cjdb --force` has served a stale, pre-patch build from a path
rebuilt with new content; only a full `uv cache clean` or a new path picks up
the change). A fixed output path would risk
