...
# (`benchmark/readbench/src/coordinator.rs` warns about a missing
...
# `Format::ALL` in `benchmark/readbench/src/format.rs`; this is a
...
# `benchmark/readbench/src/naming.rs` and in the justfile. Keep the
# array on ONE line and the function's closing brace in column 1:
# `benchmark/readbench/tests/strip_extension.rs` extracts both out of
...
# benchmark/scripts/fetch_tools.sh, which unpacks it under benchmark/formats/tools/ behind a
...
if [[ "$INPUT_KIND" == "citygml" ]] \
  ...
  if [[ "$BUILD_CITYGML" -eq 1 ]]; then
    ...
    #    crates/core/src/source.rs rejects everything else), while
...
# benchmark/readbench/src/format.rs — the coordinator resolves them
...
if [[ "$NEED_CLI" -eq 1 ]]; then
  ...
  # The converter is the LIBRARY's binary, not the benchmark's: this script
  # lives under benchmark/ but has to reach into lib/cityparquet-rs to build it.
  ( cd "$MONO_ROOT/lib/cityparquet-rs" && cargo build --release -p cityparquet-cli )
  CITYPARQUET="$MONO_ROOT/lib/cityparquet-rs/target/release/cityparquet"
