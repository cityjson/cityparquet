...
# family tables). `benchmark/scripts/package_tables.py` resolves the table list from the
...
MAIN_TABLE_NAME="$("$BENCHMARK_DIR/scripts/package_tables.py" "$PARQUET_PKG" --single)"
...
if [[ -n "$ATTR_COLUMN" ]]; then
  ...
  RSS=$(capture_rss "$SQL")
  SEL=$(safe_div "$MATCHES" "$TOTAL")
  append_row "$DATASET" "duckdb-parquet" "attr-filter" "$SEL" "$MATCHES" "$TIME_S" "$TIME_STD_S" "" "$RSS" "$REPEAT" "$NOTES"
else
