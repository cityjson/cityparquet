...
to `benchmark/formats/archive/2026-08-17-catalogue-corpus/` rather than left in the
results directories where `just plot` would chart them as current. Re-populate
`benchmark/formats/read_results/` and `benchmark/formats/ordering_results/` with the Reproduce recipes
