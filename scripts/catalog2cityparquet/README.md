...
| `duplicate_bundle`             | conformance     | Skipped before downloading: Japan's 381 whole-city ZIPs repackage the same data as the 60,090 per-module tiles, and converting both would encode Japan twice. A bundle named with `--item` is converted instead: a run that selects its items has no tiles to duplicate.       |
...
# or: uv run --project scripts/catalog2cityparquet python -m catalog2cityparquet \
