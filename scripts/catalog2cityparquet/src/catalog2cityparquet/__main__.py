...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    # Both defaults are relative to the REPOSITORY ROOT, which is where the
    # `catalog-*` recipes run from. `just catalog-tools` builds the two binaries
    # at exactly these paths; this driver interprets no city model itself.
    parser.add_argument(
        "--binary",
        type=Path,
        default=Path("lib/cityparquet-rs/target/release/cityparquet"),
    )
    parser.add_argument(
        "--tool",
        type=Path,
