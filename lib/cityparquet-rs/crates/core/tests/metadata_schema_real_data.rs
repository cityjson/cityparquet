...
//! (`crates/schema/assets/city.schema.json`, mirroring how
...
/// spec-as-code artifacts (`crates/schema/assets/`), not under
...
fn city_schema() -> Validator {
    let p = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../schema/assets/city.schema.json");
    assert!(p.exists(), "missing crates/schema/assets/city.schema.json");
    let schema: Value =
