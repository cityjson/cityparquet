...
/// `crates/core/tests/convert_real_data.rs`). This run is strict
...
fn export_package_to_gml_writes_citygml() {
    ...
    let ingolstadt = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../core/tests/data/savenow_ingolstadt_lod2.gml");
...
/// `crates/core/tests/convert_real_data.rs`'s helper of the same name.
...
/// `crates/core/tests/convert_real_data.rs::dangling_material_variant`
