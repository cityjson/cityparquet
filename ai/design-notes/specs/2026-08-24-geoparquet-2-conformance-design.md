...
**Date:** 2026-08-24
**Status:** design, approved in chat. Working document, deliberately uncommitted — it
goes stale as the implementation lands.
...
Each gates the module named. Resolve before implementing it, not during.

1. **Does parquet-rs's WKB statistics accumulator tolerate type code 1015?** The
   `parquet-geospatial` crate is not vendored locally (the feature is off, so it was
   never fetched), so this is unverified. If the accumulator errors or silently drops
   unknown type codes, the solid-column half of the conformance model needs rework —
   most likely to "annotate, but suppress statistics on solid columns". **Gates B2.**
2. **Does `parquet_geospatial::WkbType` emit the same `geoarrow.wkb` Arrow extension
   name as `geoarrow_schema::WkbType`?** If yes, the `--geoarrow` flag and the 2.0 path
   unify and `geoarrow-schema` may become removable. If no, decide which tag the Arrow
   schema carries — and note that the flag's contract is Arrow-level self-description,
   which is not what the Parquet logical type provides. **Gates B2.**
3. **Which `GeoParquetVersion` values emit the native logical type, and does
   `'none'` still emit it on `GEOMETRY`-typed columns in vendored DuckDB v1.5.4?**
   The code reading above suggests the logical type follows the column type rather
   than the version setting, but that is inferred from one call site, not observed.
   Confirm on a real written file. **Gates B3.**
