...
- **duckdb-cityjson**: Community extension that handles reading/writing CityJSON, CityJSONSeq, and FlatCityBuf files natively via SQL functions (`read_cityjson()`, `read_cityjsonseq()`, `read_flatcitybuf()`, `COPY TO` with CityJSON formats, `cityjson_metadata()`, `cityjsonseq_metadata()`). Install: `INSTALL cityjson FROM community; LOAD cityjson;`
...
- **No manual CityJSON parsing**: The cityjson extension handles all format reading/writing. Do NOT manually parse CityJSON fields or write temp JSON files for insertion.
